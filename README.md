...
`.github/workflows/daily_crawl.yml` 每天约在**贝尔格莱德时间 19:00**运行（UTC 17:00；冬令时会偏到当地 18:00），也可在 Actions 页手动 Run workflow。若当日报告已存在，定时任务会跳过以免重复发信。仓库需配置 Secret：`KIMI_API_KEY`。
