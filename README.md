...
3. Kimi 判定是否符合在野党口径；符合则按使馆**内政简报体**写一条概括标题 + 约500字单段摘要（专名用中文标准译法，SNS=前进党；去重只留关键信息）  
4. 写入 `reports/report_YYYY-MM-DD.docx`（标题、正文、来源、原文链接）
...
`.github/workflows/daily_crawl.yml` 每天约在**贝尔格莱德时间 19:00**运行（UTC 17:00；冬令时会偏到当地 18:00），也可在 Actions 页手动 Run workflow（代码 push 不自动跑）。若当日报告已存在，定时任务会跳过以免重复发信。仓库需配置 Secret：`KIMI_API_KEY`。
